				...
				<div class="ui-card-wrap">
						...
						<div class="col-lg-12 col-md-12">
							...
							<div class="card">
								<div class="card-main">
									<div class="card-inner">
										...
										<p>严禁 BT 下载、垃圾邮件发送以及其它为列出的非法活动，一经发现，我方有权在不通知用户且不退款的情况下查封或删除账号！</p>
									</div>
							...
							<div class="card">
								<div class="card-main">
									<div class="card-inner">
										<p class="card-heading">使用流程：</p>
										<p><a href="/user/code">充值</a> → <a href="/user/shop">购买套餐</a> → <a href="/user/node">安装工具 & 扫码</a></p>
									</div>
								</div>
							</div>
							...
							<div class="card">
								<div class="card-main">
									<div class="card-inner margin-bottom-no">
										...
										<p>您可以在这里查看您的连接信息。<br>同时，这里为您提供了自动化的配置文件生成，包含了所有 Shadowsocks 服务器的信息，方便您在诸多的服务器中快速添加，快速切换。</p>
											...
											<div class="tab-content">
												<div class="tab-pane fade {if $ssr_prefer}active in{/if}" id="all_ssr">
													...
													<div class="tab-pane fade active in" id="all_ssr_info">
														...
														{else}
															<p>您目前的加密方式、混淆或协议设置在 ShadowsocksR 客户端下无法连接，请您选用 Shadowsocks 客户端来连接，或者到<a href="/user/edit">修改资料</a>页面更新设置后再来此处查看。</p>

															<p>同时，ShadowsocksR 单端口多用户的连接不受您的设置影响，您可以在此使用相应的客户端进行连接。</p>

															<p>请注意，在当前状态下您的 SSR 订阅链接已经失效，您无法通过此种方式导入节点。</p>
														{/if}
													</div>
													<div class="tab-pane fade" id="all_ssr_windows">
														<p><a href="https://github.com/shadowsocksr-backup/shadowsocksr-csharp/releases" target="_blank">下载客户端</a>，解压，运行程序，然后您有三种方式导入所有节点<br>
															(1)下载<a href="/user/getpcconf?is_mu=0&is_ss=0">订阅地址（普通端口）</a>或者<a href="/user/getpcconf?is_mu=1&is_ss=0">订阅地址（单端口多用户）</a>，右键小飞机-->服务器-->从配置文件导入服务器，并导入已下载的订阅地址文件；<br>
															(2)点击<a class="copy-text" data-clipboard-text="{$ssr_url_all}">这里（普通端口）</a>或者<a class="copy-text" data-clipboard-text="{$ssr_url_all}">这个（单端口多用户）</a>，右键小飞机-->从剪贴板复制地址；<br>
															(3)(推荐)右键小飞机-->服务器-->SSR服务器订阅设置，将订阅地址设置为下面的地址，其他参数留空，确定之后再更新 SSR 服务器订阅。<br>
															执行以上其中一项之后，选择一个合适的服务器，将系统代理模式设定为“全局模式”，代理规则设定为“绕过局域网和大陆”即可。</p>
													...
													<div class="tab-pane fade" id="all_ssr_mac">
														<p><a href="https://github.com/shadowsocksr-backup/ShadowsocksX-NG" target="_blank">下载客户端</a>，安装，然后下载<a href="/user/getpcconf?is_mu=0&is_ss=0">订阅地址（普通端口）</a>或者<a href="/user/getpcconf?is_mu=1&is_ss=0">订阅地址（单端口多用户）</a>，运行程序，右键小飞机-->服务器列表-->导入服务器配置文件，并导入已下载的订阅地址文件，然后选择一个合适的服务器，更新一下PAC，最后开启系统代理即可。</p>
													</div>
													<div class="tab-pane fade" id="all_ssr_ios">
														<p>推荐下载<a href="https://itunes.apple.com/cn/app/shadowrocket/id932747118" target="_blank">Shadowrocket</a>，然后在 Safari 中点击<a href="{$ssr_url_all}">这个（普通端口）</a>或者<a href="{$ssr_url_all_mu}">这个（单端口多用户）</a>，点击确定，就可以批量添加节点。</p>
														<p>SSR 订阅地址：<br>
															普通端口地址：<code>{$baseUrl}/link/{$ssr_sub_token}?mu=0</code><br>
															单端口多用户端口地址：<code>{$baseUrl}/link/{$ssr_sub_token}?mu=1</code>
														</p>
													...
													<div class="tab-pane fade" id="all_ssr_android">
														<p><a href="https://github.com/shadowsocksr-backup/shadowsocksr-android/releases" target="_blank">下载客户端</a>，安装，然后在手机上默认浏览器中点击<a href="{$ssr_url_all}">这个（普通端口）</a>或者<a href="{$ssr_url_all_mu}">这个（单端口多用户）</a>，然后点击确定，批量添加完节点，然后路由选择绕过大陆，最后右上角开启即可。同时提供一个 ACL 地址，<a href="/link/{$acl_token}">复制地址</a>并到客户端里应用即可。</p>
													...
													<div class="tab-pane fade" id="all_ssr_router">
														<p>路由器 刷入<a href="https://bitbucket.org/padavan/rt-n56u/downloads/" target="_blank">该固件</a>，然后 SSH 登陆路由器，执行以下命令导入（普通端口）<br>
														...
														执行完毕后就可以到路由器的设置面板里选择 Shadowsocks 服务器进行连接了。</p>
													</div>
												...
												<div class="tab-pane fade {if !$ssr_prefer}active in{/if}" id="all_ss">
													...
													<div class="tab-pane fade active in" id="all_ss_info">
														...
														{else}
															<p>您目前的加密方式、混淆或协议设置在 Shadowsocks 客户端下无法连接，请您选用 ShadowsocksR 客户端来连接，或者到<a href="/user/edit">修改资料</a>页面更新设置后再来此处查看。</p>
															<p>同时，Shadowsocks 单端口多用户的连接不受您的设置影响，您可以在此使用相应的客户端进行连接。</p>
														{/if}
													</div>
													<div class="tab-pane fade" id="all_ss_windows">
														<p><a href="https://github.com/shadowsocks/shadowsocks-windows/releases" target="_blank">下载客户端</a>，解压，运行程序，然后您有两种方式导入所有节点<br>
													...
													<div class="tab-pane fade" id="all_ss_mac">
														<p><a href="https://github.com/shadowsocks/ShadowsocksX-NG" target="_blank">下载客户端</a>，安装，然后下载<a href="/user/getpcconf?is_mu=0&is_ss=1">这个（普通端口）</a>或者<a href="/user/getpcconf?is_mu=1&is_ss=1">这个（单端口多用户）</a>，运行程序，右键小飞机-->服务器列表-->导入服务器配置文件，并导入已下载的订阅地址文件，然后选择一个合适的服务器，更新一下PAC，最后开启系统代理即可。</p>
													</div>
													<div class="tab-pane fade" id="all_ss_ios">
														<p>推荐下载<a href="https://itunes.apple.com/cn/app/shadowrocket/id932747118" target="_blank">Shadowrocket</a>，然后在 Safari 中点击<a href="{$ss_url_all}">这个（普通端口）</a>或者<a href="{$ss_url_all_mu}">这个（单端口多用户）</a>，点击确定，就可以批量添加节点。</p>
														<p>iOS 下载<a href="/link/{$ios_token}?is_ss=1">这个（普通端口）</a>或者<a href="/link/{$ios_token}?is_ss=1&is_mu=1">这个（单端口多用户）</a>，导入到 Surge 中，最后切换适合的服务器即可。</p>
													</div>
													<div class="tab-pane fade" id="all_ss_android">
														<p><a href="https://github.com/shadowsocks/shadowsocks-android/releases" target="_blank">下载客户端</a>，然后在手机上点击<a class="copy-text" data-clipboard-text="{$ss_url_all}">这个链接（普通端口）</a>或者<a class="copy-text" data-clipboard-text="{$ss_url_all_mu}">这个链接（单端口多用户端口）</a>复制到剪贴板，打开 Shadowsocks 客户端，选择“从剪贴板导入”，然后选择一个节点，设置路由为“绕过局域网与中国大陆 IP 地址”，最后右上角开启即可。</p>
													</div>
